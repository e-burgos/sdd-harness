...
export const infoCommand = defineCommand({
  ...
  async run() {
    ...
    if (existsSync(globalPath)) {
      ...
      const moduleNames = (modules: Array<{ module?: string }> | undefined) =>
        (modules ?? []).map((m) => m.module).filter(Boolean).join(', ') || 'none';
      p.note(
        [
          `${pc.bold('Project:')} ${global.project || 'unknown'}`,
          `${pc.bold('In progress:')} ${moduleNames(global.in_progress_modules)}`,
          `${pc.bold('Pending:')} ${moduleNames(global.pending_modules)}`,
          `${pc.bold('Completed:')} ${moduleNames(global.completed_modules)}`,
          `${pc.bold('Apps:')} ${Object.keys(global.monorepo?.apps ?? {}).join(', ') || 'none'}`,
        ].join('\n'),
        'SDD Status',
      );
    } else {
